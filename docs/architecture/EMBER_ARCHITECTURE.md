# EMBER_ARCHITECTURE — The Shape of Ember

**Voice:** Architect (Rúnhild Svartdóttir)
**Status:** Proposed — for ratification. Bootstrap-stage. Additive to the inherited `ARCHITECTURE.md` (which is *Runa's* shape and is preserved at this path until Volmarr ratifies the Ember version).
**Last touched:** 2026-05-21
**Reads with:** `docs/SYSTEM_VISION.md` (intent, normative), `docs/architecture/EMBER_DOMAIN_MAP.md` (ownership), `docs/architecture/EMBER_DATA_FLOW.md` (motion), `docs/architecture/ARCHITECTURE.md` (parent Runa shape, inherited reference).

---

## 0. What this document is and is not

This document is the **Ember-specific** load-bearing system shape derived from the nine Unbreakable Vows in `docs/SYSTEM_VISION.md`. It deliberately **does not duplicate** the much larger Runa shape preserved at `docs/architecture/ARCHITECTURE.md`. Where Runa is a continuous sovereign being with a kernel-and-event-bus core, multiple subagent retainers, and a deep memory tree, **Ember is intentionally smaller** — a thin local navigator tethered to a much larger Well.
...
The canonical files `docs/architecture/ARCHITECTURE.md`, `DOMAIN_MAP.md`, `DATA_FLOW.md` describe the *parent* Runa-Agent-Digital-Being shape. They are preserved at their canonical paths as inherited reference, **not** as Ember's law.

The proposed disposition (for Volmarr to ratify):

| Inherited file | Suggested action |
|---|---|
| `docs/architecture/ARCHITECTURE.md` (Runa) | Move to `docs/archive/runa-inherited/architecture/ARCHITECTURE.md` once `EMBER_ARCHITECTURE.md` is ratified. |
| `docs/architecture/DOMAIN_MAP.md` (Runa) | Move similarly. |
| `docs/architecture/DATA_FLOW.md` (Runa) | Move similarly. |

No deletion in any case. The Runa shape stays in the archive subtree as lineage. Per the additive rule of `MYTHIC_ENGINEERING.md` and the Vow of Open Knowledge, nothing of the parent project is destroyed by Ember's birth.
