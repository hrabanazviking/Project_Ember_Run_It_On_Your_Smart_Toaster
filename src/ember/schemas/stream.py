...
Phase-16 extension (ADR 0011): the final chunk may also carry
``tool_calls`` parsed from the model's structured output. Munnr drives
the propose-approve-execute-feedback loop when this tuple is non-empty.
"""
...
class FuniStreamChunk:
    ...
    ``tool_calls`` populate on the final chunk only (``done=True``).
    Earlier chunks leave them ``None`` / ``0`` / ``()``.
    """
